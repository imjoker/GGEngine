...
eae6320::cResult eae6320::Graphics::View::InitializeViews (const sInitializationParameters & i_initializationParameters)
{
	...
	const unsigned int resWidth = i_initializationParameters.resolutionWidth;
	const unsigned int resHeight = i_initializationParameters.resolutionHeight;
	...
	// Create a depth/stencil buffer and a view of it
	{
		...
		{
			const auto textureDescription = [resWidth, resHeight]
			{
				...
				textureDescription.Width = resWidth;
				textureDescription.Height = resHeight;
	...
	{
		const auto viewPort = [resWidth, resHeight]
		{
			...
			viewPort.Width = static_cast<float>(resWidth);
			viewPort.Height = static_cast<float>(resHeight);
...
void eae6320::Graphics::View::PrepareViews(unsigned int pHexCode)
{
	...
	// by "clearing" the image buffer (filling it with a solid color)
	{
		...
		clearColor[0] = bgColor.R;
		clearColor[1] = bgColor.G;
		clearColor[2] = bgColor.B;
		clearColor[3] = bgColor.A;
